...
image=${image:-khulnasoft/server}
sg_dump_file=$(mktemp)
codeintel_dump_file=$(mktemp)

trap 'rm -f ${sg_dump_file} ${codeintel_dump_file}' EXIT

# Get the  ID of the khulnasoft container
echo "Obtaing khulnasoft container ID"
container_id=$(ssh "${src_host}" docker ps | grep "${image}:${version}" | cut -f 1 -d ' ')

# Dump khulnasoft db and alter script for migration
echo "Dumping khulnasoft database"
ssh ${src_host} "docker exec ${container_id} pg_dump -C --username=postgres khulnasoft" >${sg_dump_file}
...
printf '\connect -reuse-previous=on dbname=postgres\nDROP DATABASE sg;\nALTER DATABASE "khulnasoft-codeintel" RENAME TO sg;\nALTER DATABASE sg OWNER TO sg;\n' >>${codeintel_dump_file}
