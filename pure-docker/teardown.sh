...
docker rm -f khulnasoft-frontend-internal &>/dev/null || true &
docker rm -f $(addresses "khulnasoft-frontend-" $NUM_FRONTEND "") &>/dev/null || true &
...
docker network rm khulnasoft &>/dev/null || true &
wait
