...
func main() {
	app := &cli.App{
		Name:  "upgrade-test",
		Usage: "Upgrade test is an upgrade smoke test for Khulnasoft's docker-compose deployment type.",
		Flags: []cli.Flag{
			&cli.BoolFlag{
				Name:    "verbose",
				Aliases: []string{"v"},
			...
			},
			{
				...
				Flags: []cli.Flag{
					&cli.StringFlag{
						Name:     "from",
						Aliases:  []string{"f"},
						Usage:    "Khulnasoftversion to upgrade from",
						Required: true,
					},
					&cli.StringFlag{
						Name:     "to",
						Aliases:  []string{"t"},
						Usage:    "Khulnasoftversion to upgrade to",
						Required: true,
					},
				},
				Action: func(cCtx *cli.Context) error {
...
// initTestEnv
// - clones the deploy-khulnasoft-docker repo into a temp directory,
...
func initTestEnv(ctx context.Context, verbose bool) (*testEnv, error) {
	...
	// Clone the deploy-khulnasoft-docker repo into the tmp directory
	if verbose {
		err := run.Cmd(ctx, "git", "clone", "https://github.com/khulnasoft/deploy-khulnasoft-docker.git").
			Dir(tmpDir).
			Run().Stream(os.Stdout)
		if err != nil {
			return nil, fmt.Errorf("failed to clone deploy-khulnasoft-docker repo: %w", err)
		}
		fmt.Println("Cloned deploy-khulnasoft-docker repo to: ", filepath.Join(tmpDir, "deploy-khulnasoft-docker"))
	} else {
		if err := run.Cmd(ctx, "git", "clone", "https://github.com/khulnasoft/deploy-khulnasoft-docker.git").
	...
	return &testEnv{
		ctx:     ctx,
		tmpDir:  tmpDir,
		dcDir:   filepath.Join(tmpDir, "deploy-khulnasoft-docker", "docker-compose"),
		verbose: verbose,
		cleanup: cleanup,
	}, nil
}
...
func migratorBaseString(ctx context.Context, migratorVersion *semver.Version) []string {
	migratorBase := []string{"docker", "run", "--rm",
		...
		"--network=docker-compose_khulnasoft",
		"khulnasoft/migrator:" + migratorVersion.String(),
	}
	return migratorBase
}
